...
class RawImagePatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        noisy = RawHandler(row["noisy_image"])
        gt = RawHandler(row["gt_image"])
        ...
        noisy_patch = noisy.as_rgb(colorspace=self.colorspace, dims=crop_dim).astype(np.float32)
        align_crop_dim = (y1-align_offset, y2+align_offset, x1-align_offset, x2+align_offset)
        gt_patch = gt.as_rgb(colorspace=self.colorspace, dims=align_crop_dim).astype(np.float32)
        ...
        # RGGB 
        noisy_bayer = noisy.apply_colorspace_transform(colorspace=self.colorspace, dims=crop_dim)
        noisy_rggb = pixel_unshuffle(noisy_bayer, 2)
